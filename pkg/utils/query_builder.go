...
func (q *queryBuilder) CopyQuery() QueryBuilderInteractor {
	copyQuery := *q
	return &copyQuery
}
...
func (q *queryBuilder) InsertQuery(tableName string, arrColumn []string, models map[string]interface{}) (stmt string, values []interface{}, err error) {
	...
	for key, val := range arrColumn {
		...
		stmtValues = fmt.Sprintf(`%s ?`, stmtValues)
		values = append(values, models[val])
		...
		if key < lastIndex {
			...
			stmt += `)`
			stmtValues += `)`
			stmt = fmt.Sprintf(`%s %s`, stmt, stmtValues)
		}
	}
...
func (q *queryBuilder) InsertManyQuery(tableName string, arrColumn []string, models []interface{}) (stmt string, values []interface{}, err error) {
	stmt = fmt.Sprintf(`INSERT INTO %s (`, tableName)
	stmtValues := "VALUES "
	lastIndexCol := len(arrColumn) - 1
	lastIndexRow := len(models) - 1
	...
	for key, val := range arrColumn {
		...
		} else if key == lastIndexCol {
			stmt += `)`

		}
	}

	for keyRow, valRow := range models {
		stmtValues += `(`
		dataModel := valRow.(map[string]interface{})
		for keyCol, valCol := range arrColumn {
			stmtValues = fmt.Sprintf(`%s ?`, stmtValues)
			values = append(values, dataModel[valCol])
			if keyCol < lastIndexCol {
				stmtValues += `,`

			}
		}
		if keyRow < lastIndexRow {
			stmtValues += `),`
		}
		if keyRow == lastIndexRow {
			stmtValues += `)`
...
func (q *queryBuilder) UpdateQuery(tableName string, arrColumn []string, models map[string]interface{}) (stmt string, values []interface{}, err error) {
	...
	for key, val := range arrColumn {
		...
		if key < lastIndex && models[val] != nil {
			stmt += `, `
		} else if key == lastIndex {
			var valuesWhere []interface{}
			where, valuesWhere, err = parseWhere(*q.where)
			if err != nil {
				logger.Logger.Error(err)
				return
			}
			stmt = fmt.Sprintf(`%s WHERE %s`, stmt, where)
...
func (q *queryBuilder) AddCount(column string, aliases string) {
	selection := fmt.Sprintf(`COUNT(%s) %s`, column, aliases)
	q.AddSelection(selection)
}
