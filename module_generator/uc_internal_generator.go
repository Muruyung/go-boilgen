...
func internalUcGenerator(dto dtoModule, isAll, isOnly bool) error {
	...
	if _, ok := dto.methods["create"]; ok {
		err = generateCreatetUc(dto.name, dto.path, dto.services, dto.fields)
		if err != nil {
			return err
		} else {
...
func generateCreatetUc(name, path, services string, fields map[string]string) error {
	var (
		file            = jen.NewFilePathName(path, strings.ToLower(name))
		upperName       = capitalize(name)
		title           = sentences(name)
	...
	file.Func().Params(jen.Id("uc").Id(embedStruct)).Id(methodName).
		...
		Block(
			...
			jen.Line(),
			jen.Id("err").Id(":=").Id("uc").Dot(svcName).Dot(methodName).Id("(ctx, service."+dto).Block(
				generatedParser...,
			).Id(")"),
			jen.If(jen.Id("err").Op("!=").Nil()).Block(
				jen.Id(loggerErr).Parens(
